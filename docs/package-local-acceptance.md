...
- 构建 universal Release `cmux.app`
- 生成 `cmax-macos-local.dmg`
- 构建并输出 `remote-daemon-assets/`
- 将同一份 `remote-daemon-assets/` 一并打进 `cmux.app/Contents/Resources/remote-daemon-assets/`
...
该 workflow 仅用于**预打包 / 验收包**，正式对外发布仍走 `release.yml`。

预打包 artifact 的 `cmux.app` 也会自带 `remote-daemon-assets`，因此本地解压后可直接验收 remote daemon bootstrap，不依赖额外 GitHub Release 资产上传。
