...
# Build, sign, notarize, create DMG, generate appcast, and upload to the fork GitHub release.
...
APP_PATH="build/Build/Products/Release/Cmax.app"
...
/usr/libexec/PlistBuddy -c "Add :SUFeedURL string https://github.com/Lcc1ccl/cmax/releases/latest/download/appcast.xml" "$APP_PLIST"
...
echo "Creating DMG..."
rm -f cmax-macos.dmg
create-dmg --codesign "$SIGN_HASH" cmax-macos.dmg "$APP_PATH"
echo "Notarizing DMG..."
xcrun notarytool submit cmax-macos.dmg \
  --apple-id "$APPLE_ID" --team-id "$APPLE_TEAM_ID" --password "$APPLE_APP_SPECIFIC_PASSWORD" --wait
xcrun stapler staple cmax-macos.dmg
xcrun stapler validate cmax-macos.dmg
echo "DMG notarized"

# --- Generate Sparkle appcast ---
echo "Generating appcast..."
./scripts/sparkle_generate_appcast.sh cmax-macos.dmg "$TAG" appcast.xml
...
if gh release view "$TAG" >/dev/null 2>&1; then
  ...
  for asset in cmax-macos.dmg appcast.xml; do
  ...
  if [[ "$ALLOW_OVERWRITE" == "true" ]]; then
    ...
    gh release upload "$TAG" cmax-macos.dmg appcast.xml --clobber
  else
    echo "Uploading to existing release $TAG..."
    gh release upload "$TAG" cmax-macos.dmg appcast.xml
  fi
else
  echo "Creating release $TAG and uploading..."
  gh release create "$TAG" cmax-macos.dmg appcast.xml --title "$TAG" --notes "See CHANGELOG.md for details"
fi
...
# --- Cleanup ---
rm -rf build/ cmax-macos.dmg appcast.xml
echo ""
echo "=== Release $TAG complete ==="
say "cmax release complete"
